...
const Sidebar: React.FC<SidebarProps> = ({ role, activePage, onNavigate, onLogout }) => {
  ...
  // 2. Bottom Navigation Items (Mobile Quick Access) - UPDATED WITH CENTRAL DASH
    ...
    <>
      ...
      {bottomItems.length > 0 && (
        <div className="md:hidden fixed bottom-6 left-4 right-4 z-40">
            ...
            */}
            <div className="bg-brand-gray-900/95 backdrop-blur-xl border border-white/10 shadow-2xl rounded-2xl flex justify-between items-end h-16 px-2 pb-1 relative">
                {bottomItems.map((item, idx) => {
                    ...
                    if (isCentral) {
                        return (
                            <div key={item.label} className="relative -top-6 flex flex-col items-center justify-start w-1/5 h-full pointer-events-none">
                                <button
                                ...
                                {/* Optional Label for Central Button (Hidden to keep clean, or shown below) */}
                                {/* <span className="text-[9px] font-bold text-brand-primary mt-1 shadow-sm bg-white/80 px-1 rounded backdrop-blur-sm">DASH</span> */}
                            </div>
                        );
                    }
                    ...
                    return (
                        <button
                            key={item.label}
                            onClick={() => onNavigate(item.page)}
                            className="relative flex flex-col items-center justify-center w-1/5 h-14 group"
                        >
                            <div className={`
