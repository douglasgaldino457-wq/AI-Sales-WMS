...
import { 
    ...
    Camera, Image as ImageIcon, Briefcase, Plus, Trash2, Smartphone, Save, UploadCloud
} from 'lucide-react';
...
const CadastroPage: React.FC<CadastroPageProps> = ({ role }) => {
    ...
    }, [isApproving, isSubmitting]); // Refresh when approved or submitted
    ...
    const handleApprove = async () => {
        ...
        setSelectedReq(null);
        alert(`Cadastro aprovado! Enviado para a fila de Logística.`);
    };
    ...
    if (viewMode === 'FORM') {
        return (
            <div className="max-w-5xl mx-auto space-y-6 pb-20">
                ...
                <header className="flex items-center justify-between border-b border-brand-gray-200 pb-4">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setViewMode('LIST')} className="p-2 hover:bg-brand-gray-100 rounded-full transition-colors">
                            <ArrowRight className="w-6 h-6 rotate-180 text-brand-gray-600" />
                        </button>
                        <div>
                            <h1 className="text-2xl font-bold text-brand-gray-900">Novo Credenciamento</h1>
                            <p className="text-brand-gray-600 text-sm">Preencha os dados completos para ativar o EC.</p>
                        </div>
                    </div>
    ...
    return (
        <div className="h-[calc(100vh-6rem)] flex flex-col md:flex-row gap-6">
            {/* ... (Admin Logic kept same as previous, just adding the "Novo" button) ... */}
            
            {/* LEFT: LIST (For Admin and Sales Tracking) */}
            <div className={`w-full ${isAdmin ? 'md:w-1/3' : 'md:w-full'} flex flex-col bg-white rounded-xl shadow-sm border border-brand-gray-100 overflow-hidden`}>
                <div className="p-4 border-b border-brand-gray-100 bg-brand-gray-50/50">
                    <div className="flex justify-between items-center mb-3">
                        <h2 className="font-bold text-brand-gray-900 text-lg flex items-center gap-2">
                            ...
                            {isAdmin ? 'Fila de Análise' : 'Meus Envios'}
                        </h2>
                        {!isAdmin && (
                            <button 
                                onClick={() => setViewMode('FORM')}
                                className="bg-brand-primary text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 hover:bg-brand-dark transition-colors shadow-sm"
                            >
                    ...
                    {/* ... Search & Filters (Same as previous) ... */}
                ...
                <div className="flex-1 overflow-y-auto divide-y divide-brand-gray-100">
                    {filteredRequests.length === 0 ? (
                        <div className="p-8 text-center text-brand-gray-400 text-xs">
                            {isAdmin ? 'Nenhum cadastro encontrado.' : 'Você ainda não enviou nenhum cadastro.'}
                        </div>
                    ) : (
                        filteredRequests.map(req => (
                            <div 
                                ...
                                className={`p-4 ${isAdmin ? 'cursor-pointer hover:bg-brand-gray-50' : ''} transition-colors border-l-4
                                    ${selectedReq?.id === req.id && isAdmin ? 'bg-brand-gray-50 border-l-brand-primary' : 'border-l-transparent'}
                                `}
                            >
                                {/* ... List Item Content ... */}
                                ...
                                <div className="flex justify-between items-center text-[10px]">
                                    ...
                                    {req.status === 'PENDING_ANALYSIS' && <span className="text-yellow-600 bg-yellow-100 px-2 py-0.5 rounded font-bold">Em Análise</span>}
            ...
            {isAdmin && (
                <div className="flex-1 bg-white rounded-xl shadow-lg border border-brand-gray-200 flex flex-col overflow-hidden relative">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-6">
                            ...
                            <div className="mb-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {/* ... Docs Checklist ... */}
                                    <div className="p-4 rounded-xl border flex items-center justify-between cursor-pointer bg-green-50 border-green-200 text-green-800 shadow-sm">
                                        <span className="font-medium text-sm">Contrato Social / MEI</span>
                                        <CheckCircle2 className="w-5 h-5 text-green-600" />
                                    </div>
                                    <div className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer ${selectedReq.docs.idCard ? 'bg-green-50 border-green-200 text-green-800' : 'bg-white border-brand-gray-200'}`}>
                                    ...
                                    <div className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer ${selectedReq.docs.addressProof ? 'bg-green-50 border-green-200 text-green-800' : 'bg-white border-brand-gray-200'}`}>
                                    ...
                                    <div className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer ${selectedReq.docs.bankProof ? 'bg-green-50 border-green-200 text-green-800' : 'bg-white border-brand-gray-200'}`}>
                                    ...
                                    {selectedReq.bankAccount?.isThirdParty && (
                                        <div className="p-4 rounded-xl border flex items-center justify-between cursor-pointer bg-yellow-50 border-yellow-200 text-yellow-800">
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-brand-gray-400">
                        ...
                        <p className="text-lg font-medium">Selecione um cadastro para analisar</p>
                    </div>
                )}
                </div>
            )}
